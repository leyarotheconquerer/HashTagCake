		...
		for (int x = 0; x < level.sizeX; x++) {
			for (int y = 0; y < level.sizeY; y++) {
				...
				if (((level.world[x,y].type == 0) && (solidBlockCount >= 4)) || 
				    ...
				    ((x >= 0 && x < level.sizeX && y >=0 && y < level.sizeY) && 
				 		(level.world[x,y].type == 1) && (level.world[x,y+1].type == 0) && 
				 		(level.world[x,y-1].type == 0) && (level.world[x+1,y].type == 0) && (level.world[x-1,y].type == 0)) ||
				    ((x == 0) || (y == 0) || (x == level.sizeX-1) || (y == level.sizeY))) {
